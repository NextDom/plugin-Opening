...
class Opening extends eqLogic
{
    ...
    public function toHtml($_version = 'dashboard')
    {
        ...
        // Si option majuscule
        if ($replace['#upperstates#'] != '0') {
            $replace['#depuisValueString#'] = mb_strtoupper($replace['#depuisValueString#']);
        }
        if ($replace['#upperstates#'] != '0') {
            $replace['#valeurduValueString#'] = mb_strtoupper($replace['#valeurduValueString#']);
        }
        if ($replace['#upperstates#'] != '0') {
            $replace['#collecteeleValueString#'] = mb_strtoupper($replace['#collecteeleValueString#']);
        }
        ...
        if ($weatherModel != '') {
            ...
            if ($shortest <= 4 && $shortest != -1) {
                ...
                // Si option majuscule
                if ($replace['#upperstatesweather#'] != '0') {
                    $replace['#weatherValueString#'] = mb_strtoupper($replace['#weatherValueString#']);
                }
                if ($replace['#upperstatesweather#'] != '0') {
                    $replace['#weatherValueDate#'] = mb_strtoupper($replace['#weatherValueDate#']);
                }
        ...
        // Si option majuscule
        if ($replace['#upperstates#'] != '0') {
            $replace['#storeValueString#'] = mb_strtoupper($replace['#storeValueString#']);
        }
        if ($replace['#upperstates#'] != '0') {
            $replace['#storeValueDate#'] = mb_strtoupper($replace['#storeValueDate#']);
        }
        ...
        // Redimensionnement
        if (is_numeric($replace['#lockSize#'])) {
            $replace['#lockSize#'] .= 'px';
        }
        ...
        // Si option majuscule
        if ($replace['#upperstates#'] != '0') {
            $replace['#lockValueString#'] = mb_strtoupper($replace['#lockValueString#']);
        }
        if ($replace['#upperstates#'] != '0') {
            $replace['#lockValueDate#'] = mb_strtoupper($replace['#lockValueDate#']);
        }
        ...
        // Si option majuscule
        if ($replace['#upperstates#'] != '0') {
            $replace['#windowValueString#'] = mb_strtoupper($replace['#windowValueString#']);
        }
        if ($replace['#upperstates#'] != '0') {
            $replace['#windowValueDate#'] = mb_strtoupper($replace['#windowValueDate#']);
        }
        ...
        // Redimensionnement		
        if (is_numeric($replace['#batterySize#'])) {
            $replace['#batterySize#'] .= 'px';
        }
        // Contruction etat textuel
        $replace['#batteryValueString#'] = __("Batterie à", __FILE__) . ' ' . ($replace['#batteryValue#']) . '%';
        // Si option majuscule
        if ($replace['#upperstates#'] != '0') {
            $replace['#batteryValueString#'] = mb_strtoupper($replace['#batteryValueString#']);
        }
        if ($replace['#upperstates#'] != '0') {
            $replace['#batteryValueDate#'] = mb_strtoupper($replace['#batteryValueDate#']);
        }
        ...
        // Redimensionnement
        if (is_numeric($replace['#motionSize#'])) {
            $replace['#motionSize#'] .= 'px';
        }
        // Contruction etat textuel
        $replace['#motionValueString#'] = (($replace['#motionValue#'] != '0') ? __("Mouvement", __FILE__) : __("Pas de mouvement", __FILE__));
        // Si option majuscule
        if ($replace['#upperstates#'] != '0') {
            $replace['#motionValueString#'] = mb_strtoupper($replace['#motionValueString#']);
        }
        if ($replace['#upperstates#'] != '0') {
            $replace['#motionValueDate#'] = mb_strtoupper($replace['#motionValueDate#']);
        }
        ...
        // Redimensionnement
        if (is_numeric($replace['#alarmSize#'])) {
            $replace['#alarmSize#'] .= 'px';
        }
        // Contruction etat textuel
        ...
        // Si option majuscule
        if ($replace['#upperstates#'] != '0') {
            $replace['#alarmValueString#'] = mb_strtoupper($replace['#alarmValueString#']);
        }
        if ($replace['#upperstates#'] != '0') {
            $replace['#alarmValueDate#'] = mb_strtoupper($replace['#alarmValueDate#']);
        }
        ...
        // Si option majuscule
        if ($replace['#upperstatestemperature#'] != '0') {
            $replace['#temperatureIntValueString#'] = mb_strtoupper($replace['#temperatureIntValueString#']);
        }
        if ($replace['#upperstatestemperature#'] != '0') {
            $replace['#temperatureExtValueString#'] = mb_strtoupper($replace['#temperatureExtValueString#']);
        }
